...
const { buildWeatherFallback, mapLocation } = require("./visitContext");
...
app.get("/weather/current", async (req, res) => {
  try {
    ...
    const weather = await loadFallbackWeather(locationId);
...
app.get("/visit-summary", async (req, res) => {
  try {
    ...
    const weather = await loadFallbackWeather(location.id);
